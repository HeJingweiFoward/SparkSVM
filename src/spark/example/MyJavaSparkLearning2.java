...
public class MyJavaSparkLearning2 {
	...
	public static void main(String[] args) throws IOException, URISyntaxException {
		...
		SparkConf sparkConf = new SparkConf().set("spark.executor.memory", "4000m").setMaster("local[4]");
...
/*		List<Tuple2<String, String>> list = mapToPairLines.collect();
		...
		//使用reduce排序
	Tuple2<String, String> maxACC=	mapToPairLines.reduce((x,y)->{if (Double.parseDouble(x._2().replace("%", ""))>Double.parseDouble(y._2().replace("%", ""))) {
			return x;
		}
		else {
			return y;
